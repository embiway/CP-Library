/*
	...
	Can compute pi(1e14) in approximately 40 seconds
...
template <const uint32_t PI_CALC = 10000000, const uint32_t PHI_M = 7, const uint32_t PHI_PM = 2 * 3 * 5 * 7 * 11 * 13 * 17>
struct CountPrimes {
	uint32_t np[(PI_CALC >> 6) + 1]; uint32_t prime[uint32_t(1.3 * PI_CALC / log(PI_CALC))], pi[PI_CALC], phi[PHI_PM + 1][PHI_M + 1], sz[PHI_M + 1];
	void init() {
		sz[0] = np[0] = 1, prime[1] = 2;
		for (uint32_t i = 3; i < sqrt(PI_CALC); i += 2) if (!(np[i >> 6] & (1 << ((i >> 1) & 31))))
			for (uint32_t j = i * i, k = i << 1; j < PI_CALC; j += k) np[j >> 6] |= (1 << ((j >> 1) & 31));
		for (uint32_t i = 2, cnt = 1; i < PI_CALC; i++) {
			if ((i & 1) && !(np[i >> 6] & (1 << ((i >> 1) & 31)))) prime[++cnt] = i;
			pi[i] = cnt;
		}
