...
struct TarjanLCA {
	...
	std::pair<int, int> ans[MAXV]; T dist[MAXV]; bool vis[MAXV]; int anc[MAXV], par[MAXV], sz[MAXV], idx = 0;
	...
	void dfs1(int cur, int prev, T d) { dist[cur] = d; for (auto &i : adj[cur]) if (i.first != prev) dfs1(i.first, cur, d + i.second); }
	void dfs2(int cur, int prev) {
		anc[cur] = cur, vis[cur] = true;
		for (auto &i : adj[cur]) if (i.first != prev) { dfs2(i.first, cur); join(cur, i.first); anc[find(cur)] = cur; }
		for (auto &i : query[cur]) if (vis[i.first]) { int a = anc[find(i.first)]; ans[i.second] = {a, dist[cur] + dist[i.first] - 2 * dist[a]}; }
	}
	void process(int root, int V = MAXV) { std::fill(vis, vis + V, false); for (int i = 0; i < V; i++) par[i] = i, sz[i] = 1; dfs1(root -= INDEXING, -1, 0); dfs2(root, -1); }
private:
